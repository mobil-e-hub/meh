...
from optimization_engine_old.helpers import load_topology, load_mapping, backtrack_shortest_path, generate_transaction_id
from optimization_engine_old.datastructures import Hub, Drone, Car, Bus, Parcel, Routes, DroneState, VehicleState, \
	TaskState, Route, Position
from mqtt_client import MQTTClient

from optimization_engine_old.modes import test_0 as mode_test_0, showcase_0 as mode_showcase_0

execution_modes = {
	('test', '0'): mode_test_0,
	('showcase', '0'): mode_showcase_0
}
...
class OptimizationEngine(MQTTClient):
	...
	def set_mode(self, mode):
		if self.mode is not None:
			if self.mode == mode:
				...
				execution_modes[self.mode].deactivate(self)
				for topic in execution_modes[self.mode].subscriptions.keys():
					self.unsubscribe_and_remove_callback(topic)

		self.mode = mode

		# Update subscriptions
		for topic, callback in execution_modes[self.mode].subscriptions.items():
			self.subscribe_and_add_callback(topic, callback)

		# Initialize new mode
		execution_modes[mode].activate(self)
	...
	def on_message_mode(self, client, userdata, msg):
		try:
			...
			if mode in execution_modes.keys():
				self.set_mode(tuple(mode))
				logging.debug(f"[{self.logging_name}] - Mode changed to {mode}")
			else:
				logging.debug(f"[{self.logging_name}] - Mode could not be changed (Invalid mode {mode})")
	...
	def unsubscribe_and_remove_callback(self, topic):
		self.unsubscribe(f"{self.project}/{self.version}/{topic}")
		self.client.message_callback_remove(f"{self.project}/{self.version}/{topic}")
