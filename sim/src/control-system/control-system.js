...
//     //     let _floydWarshall = this.floyd_warshall(nodes);
...
//     //     let s_h = this.hubSimulator.hubs[parcel.carrier.id];  // hub id //
...
//         return Object.keys(scenario.topology.nodes).find(key => scenario.topology.nodes[key].position.x === position.x && scenario.topology.nodes[key].position.y === position.y) ;
//     }
//
//
...
//     //
//     //     let node = route.path.shift();   // retrieve starting node
//     //     // let idleBuses = this.busSimulator.getBusesPassingNode(node)
//     //
//     //     let optimalCar = null;
//     //     let optimalCarIndex = 0;
//     //     let travelTime = new Array(idleCars.length); // + idleBuses.length);
//     //
//     //     let mapping = _.invert(Object.keys(topology.nodes));
//     //
//     //     let i = 0;
...
//         return Object.values(this.hubSimulator.hubs).filter(h => dist2d(scenario.topology.nodes[h.position].position, position) <= radius);
//     }
//
//     // test(message) {
